...
<draggable   handle=".drag"
                            ...
                            <div v-for="(item,index) in gui.two" :key="'guiOne'+index">
                                <div style="display:flex;justify-content:space-evenly">
                                    ...
                                    <v-text-field style="margin:1px" outlined label="Name" v-model="item.name"></v-text-field>
                                    <v-text-field style="margin:1px" outlined label="Width(%)" v-model="item.width"></v-text-field>
                                <v-icon @click="gui.two.splice(index,1)" style="font-size:30px" class="drag" >fa-trash</v-icon>                                    
...
export default {
 ...
 async mounted(){
var $vm=this;
await $vm.$store.dispatch("getGuiQasFormTwo",$vm.gui)

$vm.gui=$vm.$store.state.interplex.qas_form_two_ui;
 },
 methods:{
    async reset(){
var $vm=this;
$vm.gui=reset.dataset.qas_form_two_ui;
await $vm.$store.dispatch("setGuiQasFormTwo",$vm.gui)
$vm.$alert("Reset")
